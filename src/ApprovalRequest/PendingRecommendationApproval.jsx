            ...
            {selectedEmployee && (
                <div className="details-container">
                    ...
                    <div className="details-row">
                        <span>{selectedEmployee.firstName} {selectedEmployee.lastName} ({selectedEmployee.empCode})</span>
                        <FaTimes className="remove-icon" onClick={() => setSelectedEmployee(null)} style={{ cursor: 'pointer', color: 'red' }} />
                    </div>
                </div>
            )}
...
const PendingRecommendationApproval = () => {
    ...
    const handleTeamSearchChange = async (e) => {
        ...
        try {
            const response = await axios.get(`http://${strings.localhost}/api/employee/search?search=${value}`);
    ...
    const confirmReject = async () => {
        if (!selectedEmployee) return showToast("Select a reassignment employee", "error");
            ...
            <table className="hazoplist-table">
                ...
                <tbody>
                    {assignments.length === 0 ? (
                        <tr><td colSpan={7}>No Recommendations</td></tr>
                    ) : (
            ...
            {selectedRecord && (
                <div className="modal-overlay">
                    <div className="modal-body">
                        ...
                        {showDateInput && (
                            <div className="details-row" style={{ marginTop: '15px', background: '#f0f8ff', padding: '10px', borderRadius: '5px' }}>
                                <span className="label" style={{ fontWeight: 'bold', color: '#007bff' }}>*Target Date:</span>
                                <input
                                    type="date"
                        ...
                        <div className="confirm-buttons" style={{ marginTop: '20px' }}>
                            ...
                            {!showDateInput ? (
                                <>
                                    <button type='button' className="approveBtn" onClick={handleAcceptClick}>
                                        Accept
                                    </button>
                                    <button type='button' className="rejectBtn" onClick={() => initRejectFlow(selectedRecord.assignmentId)}>
                                        Reject
                                    </button>
                                </>
                            ) : (
                                <button type='button' className="approveBtn" onClick={handleAcceptSave}>
                                    Save & Confirm
                                </button>
                            )}
